...
public class TextMailService implements MailService{
     ...
     *
     * @param notification 이메일을 받을 사용자의 이메일 주소
     *
    ...
    public void sendEmail(Notification notification) {
        log.info("start to send mail : {}",notification);
        MimeMessage message = javaMailSender.createMimeMessage();
        MimeMessageHelper mimeMessageHelper;

        try{
            mimeMessageHelper = new MimeMessageHelper(message, false, "UTF-8");
            mimeMessageHelper.setTo(notification.getUserEmail());
            mimeMessageHelper.setSubject(notification.makeSubject());
            mimeMessageHelper.setText(notification.makeText());
            javaMailSender.send(message);
            log.info("sent mail to : {}", notification);
